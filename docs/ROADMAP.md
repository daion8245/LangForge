...
```text
[x] 4종 모두 연결 테스트가 동작한다
[x] 4종 모두 오류 분류가 TranslationError 로 정확히 매핑된다
[x] 제공자를 바꿔도 토큰 보호·검증·병합·출력 로직이 변경되지 않는다 (계획서 §3.3)
[x] 각 제공자의 키 형식이 SensitiveFilter 로 마스킹된다
[x] 엔드포인트가 소스가 아닌 데이터 파일에 있다
```
...
| # | 작업 | 크기 |
|---|---|---|
| 9.1 | `cache.db` 스키마 — 캐시 키 8요소 (`TECHNICAL.md` 7.5) | M |
| 9.2 | 캐시 조회 → `cache` 상태 적용 | M |
| 9.3 | 캐시 종류 분리 (자동/검수완료/사용자수정) — `승인` → reviewed | M |
...
```text
[x] 같은 원문·같은 조건이면 API 를 호출하지 않는다
[x] 캐시 키 8요소 중 하나라도 다르면 캐시가 적중하지 않는다
[x] 항목 0개일 때 적중률이 '—' 로 표시된다
[x] 용어집이 번역기 호출 전에 적용된다
[x] MergePolicy 6단계 전체가 전수 테스트로 검증된다
```
...
| 10.2 | 충돌 우선순위 설정 5단계 (`TECHNICAL.md 3.4`) | M |
...
`manual` 외 네 단계는 **미리선택(preselect)** 만 합니다. `resolved = true` 는 사용자가 S11 에서 확인한 뒤에만 기록합니다. 자동 확정은 없습니다 — AC-8.5.
...
|---|---|---|
| 자동 저장 | ON | |
| 상세 로그 (FINE) | OFF | 10.7 로그 뷰어와 연동 |
| 번역 완료 시 소리·알림 | ON | |
| 재탐색 시 기존 번역 유지 | ON | AC-10.7. 끄면 재탐색이 번역을 버린다 |
| 출력 전 검사 건너뛰기 허용 | OFF | E3. 켜면 S9 에서 검사 우회 버튼이 열린다 |
...
```text
[x] PRODUCT.md 8절 항목 중 자동 검증 가능한 10개가 전부 통과
[x] Translation_Report.md 가 TECHNICAL.md 8.6 명세 전체를 담는다
       (통계 · 오류 · 경고 · 충돌 · 원문 유지 항목 · 오래된 key · 출력 파일)
[x] flutter analyze 무경고 · dart format 통과
[x] 전체 테스트 통과 (corpus 제외)
[x] 코퍼스 테스트 C-1 ~ C-8 로컬 통과 → T2~T12 회귀 없음
[x] Q4(골든 테스트) · Q7(코드 서명) 결정 완료 — 둘 다 도입하지 않음
[x] 버전 번호가 pubspec.yaml 과 코드에서 하나의 상수로 일치 (lib/app_version.dart)
[x] THIRD_PARTY_LICENSES.md 갱신 (번들 폰트 2종 포함)
[x] 릴리스 노트 작성 (CHANGELOG.md)
[x] 포터블 ZIP 빌드
[ ] U1 ~ U6 — 실제 Minecraft 1.20.1 / 1.21.1 / 1.21.4 수동 확인
[ ] 포터블 ZIP 을 Flutter SDK 없는 깨끗한 Windows 에서 실행 확인
```

마지막 두 항목은 사람이 게임과 다른 PC 에서 직접 확인해야 하며, 통과 전까지 1.0 게이트는
열리지 않습니다.
...
| Q4 | ~~골든(스크린샷) 테스트를 1.0 에 넣을 것인가~~ → **넣지 않음.** 근거는 `TECHNICAL.md` 11.7 | ✅ Phase 12 |
